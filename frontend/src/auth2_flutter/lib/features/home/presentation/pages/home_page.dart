...
class _HomePageState extends State<HomePage> {
  ...
  final summaryUrl = Uri.parse('${_getBaseUrl()}/api/health-metrics/daily-summary?date=${DateTime.now().toIso8601String().split('T')[0]}');
  final summaryResponse = await http.get(
    summaryUrl,
    headers: {'Authorization': 'Bearer ${await _getToken()}'},
  );
  if (summaryResponse.statusCode == 200) {
    final summaryData = jsonDecode(summaryResponse.body)['data'];
  }
